...
.search-bar .sico{position:absolute;right:30px;top:50%;transform:translateY(-50%);color:var(--gray);font-size:16px;pointer-events:none}
...
.btn.red{background:var(--red)}.btn.grn{background:var(--green)}
.btn.amber{background:var(--orange)}
...
const CMONTH = NOW.getMonth(); // 0-based; 10=Nov, 11=Dec
const NY     = (NOW.getFullYear()+1).toString();
...
async function yfFetch(path){
  const r=await fetch(PROXY+encodeURIComponent('https://query1.finance.yahoo.com'+path),{headers:{'x-requested-with':'XMLHttpRequest'}});
  if(!r.ok)throw new Error('HTTP '+r.status); return r.json();
}
async function yf2Fetch(path){
  const r=await fetch(PROXY+encodeURIComponent('https://query2.finance.yahoo.com'+path),{headers:{'x-requested-with':'XMLHttpRequest'}});
  if(!r.ok)throw new Error('HTTP '+r.status); return r.json();
}
...
async function yfSearch(q){
  try{
    const j=await yf2Fetch(`/v1/finance/search?q=${encodeURIComponent(q)}&quotesCount=8&newsCount=0&enableFuzzyQuery=false&enableCb=false`);
    return(j?.quotes||[]).filter(x=>x.symbol&&!['FUTURE','OPTION','CURRENCY','CRYPTOCURRENCY'].includes(x.quoteType));
  }catch(e){return[];}
}
async function openFIGILookup(isin){
  try{
    const res=await fetch('https://api.openfigi.com/v3/mapping',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify([{"idType":"ID_ISIN","idValue":isin}])});
    if(!res.ok)return[];
    const data=await res.json();
    return data?.[0]?.data||[];
  }catch(e){return[];}
}
function exchSuffix(ec){
  const M={'SG':'.SI','LN':'.L','HK':'.HK','GY':'.DE','FP':'.PA','SM':'.MC','IM':'.MI','AV':'.VI','NA':'.AS','BB':'.BR','SW':'.SW','AU':'.AX','JP':'.T','KS':'.KS','TW':'.TW','US':'','UN':'','UA':'','UW':'','UR':'','UT':'','UP':''};
  return Object.prototype.hasOwnProperty.call(M,ec)?M[ec]:undefined;
}
async function resolvePriceSource(inv){
  const ticker=(inv.ticker||'').trim().toUpperCase();
  const isin=(inv.isin||'').trim().toUpperCase();
  // 1. Cached
  if(inv.resolvedTicker&&inv.resolvedTicker!==ticker){
    try{const r=await fetchPriceDirect(inv.resolvedTicker);return{...r,resolvedTicker:inv.resolvedTicker,resolvedBy:'cached'};}
    catch(e){inv.resolvedTicker=null;}
  }
  // 2. Ticker direct
  if(ticker){
    try{const r=await fetchPriceDirect(ticker);return{...r,resolvedTicker:ticker,resolvedBy:'ticker'};}catch(e){}
    if(/^\d{4,8}$/.test(ticker)){
      try{const r=await fetchPriceDirect(ticker+'.SI');return{...r,resolvedTicker:ticker+'.SI',resolvedBy:'SGX'};}catch(e){}
    }
    const yfT=await yfSearch(ticker);
    for(const q of yfT.slice(0,4)){try{const r=await fetchPriceDirect(q.symbol);return{...r,resolvedTicker:q.symbol,resolvedBy:'ticker-search'};}catch(e){}}
  }
  // 3. ISIN → Yahoo
  if(isin){
    const yfI=await yfSearch(isin);
    for(const q of yfI.slice(0,6)){try{const r=await fetchPriceDirect(q.symbol);return{...r,resolvedTicker:q.symbol,resolvedBy:'ISIN→YF'};}catch(e){}}
    // 4. ISIN → OpenFIGI
    const figiList=await openFIGILookup(isin);
    const prefTypes=['Open-End Fund','Exchange-Traded Fund','Common Stock','Closed-End Fund'];
    figiList.sort((a,b)=>(prefTypes.indexOf(a.securityType2||a.securityType||'')===-1?99:prefTypes.indexOf(a.securityType2||a.securityType||''))-(prefTypes.indexOf(b.securityType2||b.securityType||'')===-1?99:prefTypes.indexOf(b.securityType2||b.securityType||'')));
    for(const figi of figiList.slice(0,10)){
      if(!figi.ticker)continue;
      const sfx=exchSuffix(figi.exchCode);
      if(sfx===undefined)continue;
      try{const r=await fetchPriceDirect(figi.ticker+sfx);return{...r,resolvedTicker:figi.ticker+sfx,resolvedBy:'ISIN→OpenFIGI'};}catch(e){}
    }
  }
  return null;
}
async function searchTicker(q){
  const j=await yf2Fetch(`/v1/finance/search?q=${encodeURIComponent(q)}&quotesCount=8&newsCount=0&enableFuzzyQuery=true&enableCb=false`);
  return(j?.quotes||[]).filter(x=>x.symbol&&x.quoteType&&!['FUTURE','OPTION','CURRENCY'].includes(x.quoteType))
    .map(x=>({symbol:x.symbol,name:x.shortname||x.longname||x.symbol,exchange:x.exchange||x.fullExchangeName||'',type:x.typeDisp||x.quoteType||''}));
}
function resolvedByLabel(by){
  return{'ticker':'Ticker','SGX':'SGX (.SI)','ticker-search':'Ticker search','ISIN→YF':'ISIN via Yahoo','ISIN→OpenFIGI':'ISIN via OpenFIGI','cached':'Cached'}[by]||by||'';
}

// ─── State ────────────────────────────────────────────────────────────────
let S={investments:[],dividends:[],tab:'portfolio',fCcy:'All',fAcc:'All',
       modal:null,editId:null,txId:null,
       txEditInvId:null,txEditId:null,   // editing a transaction
       divEditId:null,                    // editing a dividend
       refreshing:{},refreshingAll:false,
       acResults:[],acLoading:false,acQuery:'',acTimer:null,
       searchQuery:'',                    // portfolio search
       showAllTx:{}                       // per invId: show all transactions
      };
...
function fc(n,c,dp=2){
  const m={SGD:'S$',USD:'US$',GBP:'£',EUR:'€',HKD:'HK$'};
  const s=m[c]||(c?c+' ':'');
...
async function refreshOne(invId){
  ...
  try{
    const r=await resolvePriceSource(inv);
    if(r&&r.price){
      inv.spotPrice=r.price;inv.priceChange=r.change;inv.priceChangePct=r.changePct;
      inv.priceUpdated=Date.now();inv.priceAuto=true;
      inv.resolvedTicker=r.resolvedTicker||null;inv.resolvedBy=r.resolvedBy||null;
      save();toast('✅ '+inv.name+': '+fc(r.price,inv.currency,3)+' via '+resolvedByLabel(r.resolvedBy));
    }else{inv.priceAuto=false;save();toast('⚠️ '+inv.name+': No price found. Enter manually.');}
  }catch(e){toast('⚠️ '+e.message);}
  S.refreshing[invId]=false;render();
}
async function refreshAll(){
  const list=S.investments.filter(i=>i.ticker||i.isin);
  if(!list.length){toast('No tickers or ISINs set.');return;}
  S.refreshingAll=true;render();
  let ok=0,fail=0;
  await Promise.all(list.map(async inv=>{
    S.refreshing[inv.id]=true;
    try{
      const r=await resolvePriceSource(inv);
      if(r&&r.price){
        inv.spotPrice=r.price;inv.priceChange=r.change;inv.priceChangePct=r.changePct;
        inv.priceUpdated=Date.now();inv.priceAuto=true;
        inv.resolvedTicker=r.resolvedTicker||null;inv.resolvedBy=r.resolvedBy||null;ok++;
      }else{fail++;}
    }catch(e){fail++;}
    S.refreshing[inv.id]=false;
  }));
  S.refreshingAll=false;save();
  toast('✅ '+ok+' updated'+(fail?' · ⚠️ '+fail+' not found':'')); render();
}
...
function onNameInput(val){
  ...
  S.acTimer=setTimeout(async()=>{
    try{const res=await searchTicker(val);if(S.acQuery===val){S.acResults=res;S.acLoading=false;renderAC();}}
    catch(e){S.acLoading=false;renderAC();}
  },400);
}
function renderAC(){
  const drop=document.getElementById('ac-drop');if(!drop)return;
  if(S.acLoading){drop.innerHTML='<div class="ac-loading"><span class="spin"></span> Searching…</div>';drop.style.display='block';return;}
  if(!S.acResults.length){drop.style.display='none';return;}
  drop.innerHTML=S.acResults.map((r,i)=>`
    <div class="ac-item" onclick="pickAC(${i})">
      <div style="display:flex;justify-content:space-between"><span class="ac-name">${r.name}</span><span class="ac-sym">${r.symbol}</span></div>
      <span class="ac-ex">${r.exchange} · ${r.type}</span>
    </div>`).join('');
...
function pickAC(i){
  ...
  const excCcy={NMS:'USD',NYQ:'USD',NGM:'USD',LSE:'GBP',SES:'SGD',HKG:'HKD',FRA:'EUR',EPA:'EUR'};
  const ccy=excCcy[r.exchange]||'';if(ccy){const el=document.getElementById('fc2');if(el)el.value=ccy;}
  const excMkt={NMS:'NASDAQ',NYQ:'NYSE',NGM:'NASDAQ',LSE:'LSE',SES:'SGX',HKG:'HKEX'};
  const mkt=excMkt[r.exchange]||'Other';const mel=document.getElementById('fm');if(mel)mel.value=mkt;
  S.acResults=[];S.acLoading=false;
...
function calc(inv){
  ...
  for(const t of(inv.transactions||[])){
    const u=+t.shares||0,p=+t.price||0,f=+t.fees||0;
    if(t.type==='Buy'){cost+=u*p+f;sh+=u;}
    else if(t.type==='Sell'){const ac=sh>0?cost/sh:0;cost-=u*ac;sh-=u;}
    else if(t.type==='Rights Issue'){cost+=u*p+f;sh+=u;}
    else if(t.type==='Free Issue'){sh+=u;}
  }
...
function makePie(slices,size=140){
  ...
  for(const sl of slices){
    const a=(sl.value/total)*2*Math.PI,ea=ang+a;
    const x1=cx+r*Math.cos(ang),y1=cy+r*Math.sin(ang),x2=cx+r*Math.cos(ea),y2=cy+r*Math.sin(ea);
    const xi1=cx+ri*Math.cos(ang),yi1=cy+ri*Math.sin(ang),xi2=cx+ri*Math.cos(ea),yi2=cy+ri*Math.sin(ea);
    const lg=a>Math.PI?1:0;
    paths+=`<path d="M${xi1},${yi1}L${x1},${y1}A${r},${r} 0 ${lg},1 ${x2},${y2}L${xi2},${yi2}A${ri},${ri} 0 ${lg},0 ${xi1},${yi1}" fill="${sl.color}" stroke="#fff" stroke-width="2"/>`;
    ang=ea;
  }
...
function yearEndBanner(){
  // Show in November (10) and December (11)
  if(CMONTH<10)return'';
  const allStats=S.investments.map(i=>calc(i));
  const totalDiv=allStats.reduce((s,x)=>s+x.dtot,0);
  const curDiv=allStats.reduce((s,x)=>s+x.dcur,0);
  const daysLeft=Math.ceil((new Date(NOW.getFullYear(),11,31)-NOW)/(1000*60*60*24));
  return`
  <div class="yr-banner">
    <div class="yb-ico">🔔</div>
    <div class="yb-txt">
      <b>Year-end approaching — ${daysLeft} day${daysLeft!==1?'s':''} left in ${CY}</b><br>
      On <b>1 Jan ${NY}</b>, your <b>${CY} dividends (${fc(curDiv,'SGD')})</b> will automatically become
      <b>Accumulated B/F</b> for ${NY}. Total accumulated to carry forward: <b>${fc(totalDiv,'SGD')}</b>.
      No action needed — the tracker updates itself on New Year's Day.
    </div>
  </div>`;
}
...
<div class="header">
  <div style="display:flex;justify-content:space-between;align-items:center">
    <div>
      <h1>📊 InvestTracker</h1>
      <div class="sub">${NOW.toLocaleDateString('en-SG',{weekday:'short',day:'numeric',month:'short',year:'numeric'})}</div>
    </div>
...
// Re-render only the holdings list without full page rebuild (for search perf)
function renderPortOnly(){
  ...
  if(countEl)countEl.textContent=`Holdings (${invs.length})`;
  el.innerHTML=invs.length===0
    ?`<div class="${q?'no-results':'empty'}">${q?`<div>No investments match "<b>${q}</b>"</div>`:'<div class="ico">📈</div><div>No investments yet.<br>Tap <b>+ Add</b> to begin.</div>'}</div>`
    :invs.map(inv=>renderCard(inv)).join('');
  // Re-bind spot price inputs
  document.querySelectorAll('.spi').forEach(el=>el.addEventListener('change',function(){
    const inv=S.investments.find(i=>i.id===this.dataset.id);
...
function renderCard(inv){
  const s=calc(inv);
  const loading=S.refreshing[inv.id];
  const chg=inv.priceChange||0,chgP=inv.priceChangePct||0;
  const cc=chg>=0?'var(--green)':'var(--red)';
  let srcBadge='';
  if(inv.priceAuto&&inv.spotPrice){
    if(inv.resolvedBy==='ISIN→OpenFIGI')srcBadge='<span class="fbadge">FIGI</span>';
    else if(inv.resolvedBy&&inv.resolvedBy.includes('ISIN'))srcBadge='<span class="ibadge">ISIN</span>';
    else srcBadge='<span class="abadge">LIVE</span>';
  }else{srcBadge='<span class="mbadge">MANUAL</span>';}

  const txs=inv.transactions||[];
  const showAll=S.showAllTx[inv.id];
  const displayTxs=showAll?[...txs].reverse():[...txs].reverse().slice(0,3);
  ...
  <div class="inv-top">
    <div class="inv-meta">
      ...
      <div class="inv-tkr">
        ${inv.ticker?`<b>${inv.ticker}</b>`:'No ticker'}
        ${inv.isin?` · <span style="color:var(--blue)">${inv.isin}</span>`:''}
        · ${inv.market||'–'}
      </div>
      <div class="badges">
    ...
    <div class="price-box">
      <div class="price-lbl">
        ${srcBadge}
        <button class="btn sm out" style="padding:2px 8px;font-size:10px" onclick="refreshOne('${inv.id}')" ${loading?'disabled':''}>
          ${loading?'<span class="spin"></span>':'🔄'}
        </button>
      </div>
      ${inv.spotPrice
        ?`<div class="price-big">${fc(inv.spotPrice,inv.currency,3)}</div>
          ${inv.priceAuto?`<div class="price-chg" style="color:${cc}">${chg>=0?'+':''}${fc(chg,inv.currency,3)} (${chg>=0?'+':''}${fmt(chgP,2)}%)</div>`:''}
          <div class="price-ts">${tsAgo(inv.priceUpdated)}</div>
          ${inv.resolvedTicker&&inv.resolvedTicker!==(inv.ticker||'').toUpperCase()?`<div class="price-resolved">via ${inv.resolvedTicker}</div>`:''}`
        :`<div style="font-size:11px;color:var(--gray);margin-top:6px">Tap 🔄 to fetch</div>`}
      <div class="manual-row">
        <input class="spot-inp spi" data-id="${inv.id}" type="number" step="0.0001" value="${inv.spotPrice||''}" placeholder="Manual">
      </div>
    </div>
  </div>
  ...
  <div class="grid2">
    ...
    <div class="stat"><div class="sl">Unreal. P&L</div>
      <div class="sv ${s.pl>=0?'pos':'neg'}">${s.pl>=0?'+':''}${fc(s.pl,inv.currency)}<br>
      <small>${s.pl>=0?'+':''}${fmt(s.plp,2)}%</small></div></div>
    <div class="stat"><div class="sl">Breakeven</div>
      <div class="sv" style="font-size:11px">${fc(s.avg,inv.currency,4)}<br>
      <small style="color:var(--gray)">avg cost/unit</small></div></div>
  </div>
  <div class="div-strip">
    <div class="div-strip-title">💰 Dividends Received</div>
    <div class="div-strip-grid">
      <div class="div-col">
        <div class="dc-lbl">Accum. B/F</div>
        <div class="dc-val" style="color:#0369a1">${fc(s.dbf,inv.currency)}</div>
        <div class="dc-sub">Before ${CY}</div>
      </div>
      <div class="div-col">
        <div class="dc-lbl">Year ${CY}</div>
        <div class="dc-val">${fc(s.dcur,inv.currency)}</div>
        <div class="dc-sub">This year</div>
      </div>
      <div class="div-col">
        <div class="dc-lbl">Accum. To-Date</div>
        <div class="dc-val" style="color:var(--purple)">${fc(s.dtot,inv.currency)}</div>
        <div class="dc-sub">All time</div>
      </div>
    </div>
    <div class="yield-row">
      <div class="yield-item">
        <div class="yl">Yield ${CY}</div>
        <div class="yv" style="color:var(--green)">${fmt(s.dy,2)}%</div>
      </div>
      <div class="yield-item" style="border-left:1px solid #d1fae5;border-right:1px solid #d1fae5">
        <div class="yl">Yield Total</div>
        <div class="yv" style="color:var(--purple)">${fmt(s.dytot,2)}%</div>
      </div>
      <div class="yield-item">
        <div class="yl">Div Entries</div>
        <div class="yv" style="color:var(--gray)">${S.dividends.filter(d=>d.invId===inv.id).length}</div>
      </div>
    </div>
  </div>
  <div class="actions">
  ...
  <div class="tx-section">
    ...
    ${displayTxs.map(t=>`
      <div class="tx-item">
        <div class="tx-left">
          <span class="tx-t ${t.type.toLowerCase().replace(' issue','').replace(' ','')}">${t.type}</span>
          <span style="color:var(--gray);margin-left:6px">${t.date||''}</span>
          ${t.notes?`<div style="font-size:10px;color:var(--gray);margin-top:1px">${t.notes}</div>`:''}
        </div>
        <div class="tx-right">
          <div>${fmt(t.shares,4)} units</div>
          ${t.price?`<div style="color:var(--gray);font-size:11px">@ ${fc(t.price,inv.currency,4)}</div>`:''}
          ${t.fees?`<div style="color:var(--gray);font-size:10px">fee ${fc(t.fees,inv.currency,2)}</div>`:''}
        </div>
        <div class="tx-btns">
...
<div class="section" style="padding-top:12px">
  <div class="sec-hdr"><span class="sec-ttl">Dividend Ledger</span>
  ...
  ${Object.entries(byInv).map(([id,ds])=>{
    ...
    return`<div class="ledger-inv">
      <div class="ledger-inv-hdr">
        <span>${nm} <span style="font-size:11px;color:var(--gray);font-weight:500">${inv?.ticker||''}</span></span>
      </div>
      ...
      ${Object.entries(byYr).sort((a,b)=>b[0].localeCompare(a[0])).map(([yr,yds])=>{
        ...
        return`<div class="div-yr-block">
          <div class="div-yr-hdr">
            <span>📅 ${yr} <span style="font-weight:400">(${yds.length})</span></span>
            <span></span><span></span>
            <span style="color:var(--green);font-weight:700">${fc(ytot,ccy)}</span>
            <span></span>
          </div>
          ${yds.map(d=>`<div class="div-yr-row">
            <span>${d.date||'–'}</span>
            <span style="font-size:10px;color:var(--gray)">${d.divType||'Cash'}</span>
...
function renderSummary(){
  ...
  const totalMv=allStats.reduce((s,x)=>s+x.s.mv,0),totalCost=allStats.reduce((s,x)=>s+x.s.cost,0);
  const totalPl=totalMv-totalCost,totalDiv=allStats.reduce((s,x)=>s+x.s.dtot,0);
  ...
  const topHoldings=[...allStats].sort((a,b)=>b.s.mv-a.s.mv).slice(0,6);
  const maxMv=topHoldings[0]?.s.mv||1;
  return`
<div class="summary">
...
<div class="sumpage">
  <div class="sum-section">
    ...
    <div class="pie-wrap">
      <div class="pie-title">Market Value Allocation</div>
      <div class="pie-body">
        ${makePie(typeSlices)}
        <div class="pie-legend">
          ${typeSlices.map(sl=>`<div class="pie-leg-item"><div class="pie-dot" style="background:${sl.color}"></div><span class="pie-leg-label">${sl.label}</span><span class="pie-leg-pct">${fmt(totalMv>0?sl.value/totalMv*100:0,1)}%</span></div>`).join('')}
        </div>
      </div>
    </div>
    <div class="breakdown-card">
      ...
      ${typeKeys.filter(k=>byType[k].count>0).map(k=>{const d=byType[k];return`
        <div class="bk-row" style="grid-template-columns:1.4fr 1fr 1fr 1fr">
          <div class="bk-label"><div class="bk-dot" style="background:${TYPE_COLORS[k]}"></div>${k}</div>
          <div>${fc(d.mv,'SGD',0)}<div style="font-size:10px;color:var(--gray)">${d.count} holding${d.count>1?'s':''}</div></div>
          <div style="color:${d.pl>=0?'var(--green)':'var(--red)'};font-weight:700">${d.pl>=0?'+':''}${fc(d.pl,'SGD',0)}</div>
          <div style="color:var(--purple);font-weight:700">${fc(d.div,'SGD',0)}</div>
        </div>`;}).join('')}
    </div>
  </div>
  <div class="sum-section">
    ...
    <div class="pie-wrap">
      <div class="pie-title">Market Value by Account</div>
      <div class="pie-body">
        ${makePie(accSlices)}
        <div class="pie-legend">
          ${accSlices.map(sl=>`<div class="pie-leg-item"><div class="pie-dot" style="background:${sl.color}"></div><span class="pie-leg-label">${sl.label}</span><span class="pie-leg-pct">${fmt(totalMv>0?sl.value/totalMv*100:0,1)}%</span></div>`).join('')}
        </div>
      </div>
    </div>
    <div class="breakdown-card">
      ...
      ${accKeys.filter(k=>byAcc[k].count>0).map(k=>{const d=byAcc[k];return`
        <div class="bk-row" style="grid-template-columns:1.1fr 1fr 1fr 1fr">
          <div class="bk-label"><div class="bk-dot" style="background:${ACC_COLORS[k]}"></div>${k}</div>
          <div>${fc(d.mv,'SGD',0)}<div style="font-size:10px;color:var(--gray)">${d.count} holding${d.count>1?'s':''}</div></div>
          <div style="color:${d.pl>=0?'var(--green)':'var(--red)'};font-weight:700">${d.pl>=0?'+':''}${fc(d.pl,'SGD',0)}</div>
          <div style="color:var(--purple);font-weight:700">${fc(d.div,'SGD',0)}</div>
        </div>`;}).join('')}
  ...
  <div class="sum-section">
    ...
    <div class="pie-wrap">
      <div class="pie-title">Portfolio Value by P&L Status</div>
      <div class="pie-body">
        ${makePie(profSlices)}
        <div class="pie-legend">
          ${profSlices.map(sl=>`<div class="pie-leg-item"><div class="pie-dot" style="background:${sl.color}"></div><span class="pie-leg-label">${sl.label}</span><span class="pie-leg-pct">${fmt(totalMv>0?sl.value/totalMv*100:0,1)}%</span></div>`).join('')}
        </div>
      </div>
    </div>
    <div class="breakdown-card">
      <div class="bk-hdr" style="grid-template-columns:1.8fr 1fr 1fr"><span>Investment</span><span>P&L</span><span>P&L %</span></div>
      ${[...allStats].sort((a,b)=>b.s.pl-a.s.pl).map(({inv,s})=>`
        <div class="bk-row ${s.pl>=0?'prof-winner':'prof-loser'}" style="grid-template-columns:1.8fr 1fr 1fr">
          <div><div style="font-weight:600;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${inv.name}</div>
          <div style="font-size:10px;color:var(--gray)">${inv.ticker||''} · ${inv.type}</div></div>
          <div style="color:${s.pl>=0?'var(--green)':'var(--red)'};font-weight:700">${s.pl>=0?'+':''}${fc(s.pl,inv.currency,0)}</div>
          <div style="color:${s.pl>=0?'var(--green)':'var(--red)'};font-weight:700">${s.pl>=0?'+':''}${fmt(s.plp,2)}%</div>
        </div>`).join('')}
    </div>
  </div>
  <div class="sum-section">
    ...
    <div class="breakdown-card">
      ${topHoldings.map(({inv,s})=>{const pct=totalMv>0?s.mv/totalMv*100:0,bp=maxMv>0?s.mv/maxMv*100:0,tc=TYPE_COLORS[inv.type]||'#8e8e93';return`
        <div class="holding-row">
          <div style="flex:1;min-width:0">
            <div style="display:flex;justify-content:space-between;align-items:center">
              <div style="font-weight:600;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:60%">${inv.name}</div>
              <div style="font-weight:700;font-size:13px">${fc(s.mv,inv.currency,0)}</div>
            </div>
            <div style="display:flex;justify-content:space-between;margin-top:2px">
              <div style="font-size:10px;color:var(--gray)">${inv.ticker||''} · <span style="color:${tc};font-weight:600">${inv.type}</span> · ${inv.account}</div>
              <div style="font-size:10px;color:var(--gray)">${fmt(pct,1)}%</div>
            </div>
            <div class="holding-bar-bg"><div class="holding-bar-fill" style="width:${bp}%;background:${tc}"></div></div>
          </div>
        </div>`;}).join('')}
    </div>
  </div>
...
function renderModal(){
  ...
  if(S.modal==='addInv'||S.modal==='editInv'){
    const v=S.modal==='editInv'?S.investments.find(i=>i.id===S.editId):{};
    const isnew=S.modal==='addInv';
    return`<div ${ov}><div class="modal">
      <h2>${isnew?'Add Investment':'Edit Investment'}</h2>
      <div class="fg">
        <label>Search by Name or ISIN</label>
        <input id="fsearch" placeholder="Type name, ISIN, ticker…" oninput="onNameInput(this.value)" autocomplete="off">
        <div id="ac-drop" class="ac-drop" style="display:none"></div>
        <div class="hint">Tap result → ticker, currency & market auto-filled</div>
      </div>
      ...
      <div class="m-actions">
        <button class="btn out" onclick="closeM()">Cancel</button>
        <button class="btn" onclick="saveInv()">${isnew?'Add & Fetch Price':'Save & Refresh'}</button>
      </div>
    </div></div>`;}
  ...
  if(S.modal==='addTx'){
    ...
    return`<div ${ov}><div class="modal">
      ...
      <div class="modal-sub">${inv?.name||''} · ${inv?.currency||''}</div>
      <div class="fg"><label>Type *</label><select id="ftt">${['Buy','Sell','Rights Issue','Free Issue'].map(x=>`<option>${x}</option>`).join('')}</select></div>
      <div class="row2">
        <div class="fg"><label>Date *</label><input id="ftd" type="date" value="${new Date().toISOString().slice(0,10)}"></div>
        <div class="fg"><label>Units *</label><input id="fts" type="number" step="0.0001" placeholder="0.0000"></div>
      </div>
      <div class="row2">
        <div class="fg"><label>Price / Unit</label><input id="ftp" type="number" step="0.0001" placeholder="0.0000"></div>
        <div class="fg"><label>Fees</label><input id="ftf" type="number" step="0.01" placeholder="0.00"></div>
      </div>
      <div class="fg"><label>Notes</label><input id="ftn" placeholder="Optional"></div>
      <div class="m-actions">
        <button class="btn out" onclick="closeM()">Cancel</button>
        <button class="btn" onclick="saveTx()">Add</button>
      </div>
    </div></div>`;}

  // ── Edit Transaction ──
  if(S.modal==='editTx'){
    const inv=S.investments.find(i=>i.id===S.txEditInvId);
    const tx=(inv?.transactions||[]).find(t=>t.id===S.txEditId);
    if(!tx)return'';
    return`<div ${ov}><div class="modal">
      <h2>Edit Transaction</h2>
      <div class="modal-sub">${inv?.name||''} · ${inv?.currency||''}</div>
      <div class="fg"><label>Type *</label>
        <select id="ftt">${['Buy','Sell','Rights Issue','Free Issue'].map(x=>`<option ${tx.type===x?'selected':''}>${x}</option>`).join('')}</select>
      </div>
      <div class="row2">
        <div class="fg"><label>Date *</label><input id="ftd" type="date" value="${tx.date||''}"></div>
        <div class="fg"><label>Units *</label><input id="fts" type="number" step="0.0001" value="${tx.shares||''}"></div>
      </div>
      <div class="row2">
        <div class="fg"><label>Price / Unit</label><input id="ftp" type="number" step="0.0001" value="${tx.price||''}"></div>
        <div class="fg"><label>Fees</label><input id="ftf" type="number" step="0.01" value="${tx.fees||''}"></div>
      </div>
      <div class="fg"><label>Notes</label><input id="ftn" value="${tx.notes||''}" placeholder="Optional"></div>
      <div class="m-actions">
        <button class="btn out" onclick="closeM()">Cancel</button>
        <button class="btn amber" onclick="updateTx()">Save Changes</button>
      </div>
    </div></div>`;}
  ...
  if(S.modal==='addDiv'){
    return`<div ${ov}><div class="modal">
      <h2>Add Dividend</h2>
      <div class="fg"><label>Investment *</label>
        <select id="fdi">${S.investments.map(i=>`<option value="${i.id}" ${i.id===S.txId?'selected':''}>${i.name} (${i.ticker||i.isin||i.currency})</option>`).join('')}</select>
      </div>
      <div class="row2">
        <div class="fg"><label>Date *</label><input id="fdd" type="date" value="${new Date().toISOString().slice(0,10)}"></div>
        <div class="fg"><label>Amount *</label><input id="fda" type="number" step="0.0001" placeholder="0.0000"></div>
      </div>
      ...
      <div class="m-actions">
        <button class="btn out" onclick="closeM()">Cancel</button>
        <button class="btn grn" onclick="saveDiv()">Add</button>
      </div>
    </div></div>`;}

  // ── Edit Dividend ──
  if(S.modal==='editDiv'){
    const dv=S.dividends.find(d=>d.id===S.divEditId);
    if(!dv)return'';
    const inv=S.investments.find(i=>i.id===dv.invId);
    return`<div ${ov}><div class="modal">
      <h2>Edit Dividend</h2>
      <div class="modal-sub">${inv?.name||'Unknown investment'}</div>
      <div class="fg"><label>Investment</label>
        <select id="fdi">${S.investments.map(i=>`<option value="${i.id}" ${i.id===dv.invId?'selected':''}>${i.name} (${i.ticker||i.currency})</option>`).join('')}</select>
      </div>
      <div class="row2">
        <div class="fg"><label>Date *</label><input id="fdd" type="date" value="${dv.date||''}"></div>
        <div class="fg"><label>Amount *</label><input id="fda" type="number" step="0.0001" value="${dv.amount||''}"></div>
      </div>
      <div class="fg"><label>Type</label>
        <select id="fdt">${['Cash','Scrip','Special','Return of Capital'].map(x=>`<option ${dv.divType===x?'selected':''}>${x}</option>`).join('')}</select>
      </div>
      <div class="fg"><label>Notes</label><input id="fdn" value="${dv.notes||''}" placeholder="e.g. Q1 2025 dividend"></div>
      <div class="m-actions">
        <button class="btn out" onclick="closeM()">Cancel</button>
        <button class="btn amber" onclick="updateDiv()">Save Changes</button>
      </div>
    </div></div>`;}
...
function saveInv(){
  ...
  if(!name){alert('Investment name is required.');return;}
  const ticker=document.getElementById('ftk').value.trim();
  const isin=document.getElementById('fis').value.trim();
  if(S.modal==='addInv'){
    const nv={id:uid(),name,ticker,isin,type:document.getElementById('fty').value,market:document.getElementById('fm').value,
      currency:document.getElementById('fc2').value,account:document.getElementById('fa').value,
      notes:document.getElementById('fno').value.trim(),spotPrice:0,transactions:[],resolvedTicker:null,resolvedBy:null};
    S.investments.push(nv);save();closeM();
    if(ticker||isin)setTimeout(()=>refreshOne(nv.id),300);
  }else{
    const inv=S.investments.find(i=>i.id===S.editId);
    if(inv){
      if(inv.ticker!==ticker||inv.isin!==isin){inv.resolvedTicker=null;inv.resolvedBy=null;}
      Object.assign(inv,{name,ticker,isin,type:document.getElementById('fty').value,market:document.getElementById('fm').value,
        currency:document.getElementById('fc2').value,account:document.getElementById('fa').value,notes:document.getElementById('fno').value.trim()});
    }
    save();closeM();
    if(inv&&(ticker||isin))setTimeout(()=>refreshOne(inv.id),300);
  }
}

function saveTx(){
  const sh=parseFloat(document.getElementById('fts').value);
  if(!sh||sh<=0){alert('Enter valid units.');return;}
  const inv=S.investments.find(i=>i.id===S.txId);if(!inv)return;
  if(!inv.transactions)inv.transactions=[];
  inv.transactions.push({id:uid(),type:document.getElementById('ftt').value,date:document.getElementById('ftd').value,
    shares:sh,price:parseFloat(document.getElementById('ftp').value)||0,fees:parseFloat(document.getElementById('ftf').value)||0,
    notes:document.getElementById('ftn').value.trim()});
...
function updateTx(){
  ...
  Object.assign(tx,{type:document.getElementById('ftt').value,date:document.getElementById('ftd').value,
    shares:sh,price:parseFloat(document.getElementById('ftp').value)||0,fees:parseFloat(document.getElementById('ftf').value)||0,
    notes:document.getElementById('ftn').value.trim()});
  inv.transactions.sort((a,b)=>(a.date||'').localeCompare(b.date||''));
  save();toast('✅ Transaction updated');closeM();
}

function delTx(invId,txId){
...
function saveDiv(){
  const amt=parseFloat(document.getElementById('fda').value);
  if(!amt||amt<=0){alert('Enter valid amount.');return;}
  S.dividends.push({id:uid(),invId:document.getElementById('fdi').value,date:document.getElementById('fdd').value,
    amount:amt,divType:document.getElementById('fdt').value,notes:document.getElementById('fdn').value.trim()});
  save();closeM();
}
...
function updateDiv(){
  ...
  Object.assign(dv,{invId:document.getElementById('fdi').value,date:document.getElementById('fdd').value,
    amount:amt,divType:document.getElementById('fdt').value,notes:document.getElementById('fdn').value.trim()});
...
function delDiv(divId){
  ...
  S.dividends=S.dividends.filter(d=>d.id!==divId);
  save();render();toast('🗑 Dividend deleted');
}

function delInv(id){
  if(!confirm('Delete this investment and all its transactions & dividends?'))return;
  S.investments=S.investments.filter(i=>i.id!==id);
  S.dividends=S.dividends.filter(d=>d.invId!==id);
  save();render();
}
